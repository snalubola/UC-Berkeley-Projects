...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    max_predictor = max(feature_fns, key=lambda x: find_predictor(user, reviewed, x)[1])
    return find_predictor(user, reviewed, max_predictor)[0]
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    """
    # Making a dictionary directly makes sense, but I'm not sure how exactly to implement it
    predictor = best_predictor(user, ALL_RESTAURANTS, feature_fns)
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    ratings = [user_rating(user, restaurant_name(x)) if x in reviewed else predictor(x) for x in restaurants]
    return dict(zip([restaurant_name(x) for x in restaurants], ratings))
    # END Question 9
