            ...
            if insect.name == 'Queen':
                if insect.impostor:
                    if self.ant.is_container:
                        ...
                        self.ant = None
                elif self.ant.is_container and self.ant.contained_ant == insect:
                    return
            # END Problem 13
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while counter < self.min_range:
            ...
            counter += 1
        while target_place != hive and counter <= self.max_range:
            if random_or_none(target_place.bees):
...
# BEGIN Problem 8
class WallAnt(Ant):
    """WallAnt does nothing and has more armor than other ants."""
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower acts as a normal ThrowerAnt but can be placed in water."""
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            while places_behind != None:
                if places_behind.ant:
                    if places_behind.ant not in colony.buffed_ants:
                        places_behind.ant.damage *= 2
                        colony.buffed_ants += [places_behind.ant]
                    if places_behind.ant.is_container:
                        if places_behind.ant.contained_ant and places_behind.ant.contained_ant not in colony.buffed_ants:
                            places_behind.ant.contained_ant.damage *= 2
                            colony.buffed_ants += [places_behind.ant.contained_ant]
...
def apply_effect(effect, bee, duration):
    ...
    def helper(colony):
        nonlocal effect, bee, duration, default_action, modified_action
        if duration >= 1:
            duration -= 1
