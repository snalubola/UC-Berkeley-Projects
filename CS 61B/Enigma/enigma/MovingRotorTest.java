...
public class MovingRotorTest {
    ...
    @Test
    public void checkRotorForwardBackward() {
        Permutation perm = new Permutation("(AELTPHQXRU) (BKNW) (CMOY)"
                + "(DFG) (IV) (JZ) (S)", new Alphabet());
        MovingRotor rot = new MovingRotor("I", perm, "");
        rot.set('F');
        assertEquals('I',
                rot.permutation().alphabet().toChar(rot.convertForward(5)));
        assertEquals('H',
                rot.permutation().alphabet().toChar(rot.convertBackward(9)));
    }
