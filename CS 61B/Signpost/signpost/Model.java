...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int i = 1; i < _solnNumToPlace.length; i++) {
            if (_solnNumToPlace[i].equals(0)) {
                throw new IllegalArgumentException("not all numbers appear");
            }
        }
        ...
        for (int x = 0; x < _width; x++) {
            for (int y = 0; y < _height; y++) {
                _board[x][y]._successors =
                        _allSuccessors[x][y][_board[x][y].direction()];
                for (Place pl : _board[x][y].successors()) {
    ...
    void init(int width, int height) {
        ...
        PlaceList[][][] allSuccessors = Place.successorCells(_width, _height);
    ...
    boolean autoconnect() {
        ...
        for (Sq sq1 : _allSquares) {
            for (Sq sq2 : _allSquares) {
                if (sq1.hasFixedNum() && sq2.hasFixedNum()) {
                    if (sq1.connectable(sq2)) {
                        sq1.connect(sq2);
                        changed = true;
                    } else if (sq2.connectable(sq1)) {
                        sq2.connect(sq1);
                        changed = true;
                    }
                }
    ...
    void solve() {
        for (int x = 0; x < width(); x++) {
            for (int y = 0; y < height(); y++) {
                ...
                _board[x][y]._group = 0;
                if (_board[x][y]._sequenceNum != 1) {
                    _board[x][y]._predecessor =
                            get(_solnNumToPlace[_board[x][y]._sequenceNum - 1]);
                }
                if (_board[x][y]._sequenceNum != width() * height()) {
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            if (successors().contains(s1.pl)) {
                if (successor() == null && s1.predecessor() == null) {
        ...
        boolean connect(Sq s1) {
            ...
            if (!hasFixedNum() && !s1.hasFixedNum()) {
                _head._group = joinGroups(group(), s1.group());
            } else if (hasFixedNum() && !s1.hasFixedNum()) {
                s1._sequenceNum = sequenceNum() + 1;
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                if (this._predecessor == null && next._successor == null) {
                    releaseGroup(group()); releaseGroup(next.group());
                ...
                for (Sq sq = this; sq._predecessor != null;
                     sq = sq._predecessor) {
                    if (sq._predecessor._sequenceNum != 0) {
                        counter += 1;
                    }
                }
                if (counter == 0) {
                    if (predecessor() != null) {
                        ...
                        for (Sq sq = this; sq._predecessor != null;
                            ...
                            sq._predecessor._group = newGroup;
                        }
                    } else {
                        _group = -1;
                    }
                ...
                if (counter2 == 0) {
                    if (next.successor() != null) {
                        ...
                        for (Sq sq = this; sq._successor != null;
                            ...
                            sq._successor._group = newGroup2;
                        }
                    } else {
                        next._group = -1;
                    }
        ...
        /** Locations of my possible successors. */
